...
public class FIFOUtils {
    private static Queue<PageLine> FIFOQueue  = new LinkedList<>();
    ...
    private static FIFOUtils fifoUtils;
    private FIFOUtils(){
    ...
    public static void setSequence(List<Integer> sequence) {
        FIFOUtils.sequence = sequence;
    }

    public static FIFOUtils getFifoUtils() {
        if (fifoUtils == null){
            fifoUtils = new FIFOUtils();
        }
        return fifoUtils;
    }
    ...
    public static void initAdd(PageLine pageLine){
        miss++;
        FIFOQueue.offer(pageLine);
        sequence.add(pageLine.getPageId());
        missPercent = (double)(miss)/(hit+miss);
    }
    //先进先出 置换和命中的情况
    public static void FIFO(int pageId, PCB pcb, Bitmap exBitmap){
    ...
    public static void showQueue(){
        for (PageLine pageLine:FIFOQueue){
